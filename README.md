...
## Training an ML chess bot on Modal

This repo now includes a simple neural-network chess bot and a Modal training pipeline.

### Data source

- The training script streams public human games from the [Lichess standard database](https://database.lichess.org/), which is released under CC0.
- By default it uses `lichess_db_standard_rated_2024-03.pgn.zst`, but you can override the URL with the `LICHESS_PGN_URL` environment variable.

### Model architecture

- A lightweight feedforward policy network that scores `(board, move)` pairs.
- Board features: piece planes (12 × 8 × 8), side to move, castling rights, en passant file, and normalized move count.
- Move features: from/to square coordinates, capture/promotion flags, and promotion type.

### Training on Modal

Prerequisites:
- A Modal account and the `modal` CLI installed.

Steps:

1. From the repo root (or `my-chesshacks-bot` directory), run:
   ```bash
   modal run my-chesshacks-bot/train_modal.py::train
   ```
   This will:
   - Build a Modal image with `torch`, `python-chess`, `zstandard`, and `requests`
   - Stream games from Lichess
   - Train the policy network
   - Save the weights to `/models/policy_latest.pt` in the persistent volume `chess-policy-models`

2. After training, download the model to your local repo:
   ```bash
   modal volume get chess-policy-models /models/policy_latest.pt my-chesshacks-bot/model/policy_latest.pt
   ```

3. When you run the devtools UI, `src/main.py` will automatically load the model from `my-chesshacks-bot/model/policy_latest.pt` (or from the path in `CHESS_MODEL_PATH` if set) and use it to select moves. If no model file is found, the bot falls back to a random legal move policy.
